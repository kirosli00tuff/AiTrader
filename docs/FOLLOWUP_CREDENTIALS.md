...
- Accounts/Connections page (Dash ui/ + account_manager/) must let the user TYPE and SAVE
  API keys/secrets per venue (Alpaca, Coinbase, IBKR) and per data source
  (ClankApp — free, default; SEC EDGAR — free, no key; Whale Alert — optional,
...
- Runtime resolution order: (1) in-app saved credential, else (2) env var / .env
  (the *_env names in default_config.yaml: CLANKAPP_API_KEY (optional),
