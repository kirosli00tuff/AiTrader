...
Read `docs/ARCHITECTURE.md`, `docs/DNN_ADVISORY_DESIGN.md`, and `config/default_config.yaml`
...
2. **Layer 1 (static safety) is final authority** and never bypassable by LLM / dnn_advisory /
   RL / whale / adaptive / execution. Implement as a deterministic `RiskGate` in `risk/` returning
...
4. **dnn_advisory (Layer 3)** and **whale (Layer 4)** are ADVISORY factors only. Outputs exactly
...
- `ui/` Python (Dash): the full dashboard/control board — ALL panels and ALL charts/tables
  ...
  dnn_advisory/model-verdict, smart-money/whale, risk/safety, live-approval, model-weight control).
  ...
  table, learning before/after chart, dnn_advisory perf chart, live-approval readiness panel,
