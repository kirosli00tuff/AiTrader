---
name: worker-max
description: "Worker at max effort. The lead sets the model per call: fable for the single adversarial call a stage hinges on or a disputed verification, opus for the hardest high-level work (pre-registration drafting, statistical design)."
model: fable
effort: max
---
...
- On adversarial briefs: look for every way the finding could be wrong,
  leaked, fabricated from absent input, or shaped by an outcome. Rank what
  you find by how much it would change the verdict.
