...
- **Level 1 values as shipped (re-derived 2026-07-27, previously frozen since the project began):** sizer `default_risk_per_trade_pct` **0.02**, per-trade ceiling `max_trade_risk_pct_of_equity` **0.025**, `max_open_positions_total` **10**, `max_open_positions_per_venue` **10**, `max_total_open_risk_pct` **0.25**, `max_exposure_per_category_pct` **0.25**, `max_consecutive_losses` **6**, `max_trades_per_day` **10** and `max_daily_loss_total_pct` **0.03** both unchanged. **THE SIZER AND THE CEILING ARE DISTINCT QUANTITIES and neither is derived from the other**: the sizer decides what is SENT, the ceiling decides what is PERMITTED. `max_trade_notional_cap_pct` is **UNENFORCED** and must not be cited as a ceiling. Derivations in RETURN.md 2026-07-27.
