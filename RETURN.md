...
Prompt summary: generate and test hypotheses structurally different from the four exhausted families (the RSI-2 dual-MA stack, the equity overnight premium, daily long-short time-series momentum, crypto US-session drift), under a capacity gate applied before any computation. Task 0 reject on arithmetic any candidate that cannot reach the 2,500 USD per year floor at Level 1 sizing and record every rejection with its numbers. Task 1 structural difference is a requirement and every candidate carries an economic mechanism. Task 2 pre-register the closed list, commit, then look. Task 3 score as excess over buy and hold with the fair per-unit comparison primary and volatility beside return. Task 4 the holdout is touched once. Task 5 negatives first, apply nothing, commit and push.

CONSTRAINTS HONORED: research only against analysis_bars.db. Live trading stays off. No RiskGate logic, no live-trading gate, no adaptive limit-weakening invariant, no Level 1 risk value, no strategy parameter, no threshold, no engine behavior touched. Computation sized from measured RSS under the MemoryMax scope. Queue untouched (pasted prompt path). Nothing applied.

### TASK 0 AND TASK 1, THE CAPACITY GATE AND THE STRUCTURE REQUIREMENT, applied before any backtest code

Floor: 2,500 USD per year at 100,000 equity. Level 1 sizing: notional capped at 5 percent of equity (5,000 USD per position), per-trade risk 0.5 percent. A multi-day crypto hold with a roughly 10 percent adverse-move budget sizes to about 5,000 at 0.5 percent risk, which coincides with the notional cap, so 5,000 per position is the arithmetic basis throughout. Fees per the live-schedule model: crypto 50 bp round trip taker, equity 1.3 bp.

REJECTED CANDIDATES, recorded with their numbers. Annual dollars = frequency x deployed notional x expected net move per trade.

| candidate | structure | expected move and frequency | annual USD at Level 1 | rejection |
|---|---|---|---|---|
| SEC EDGAR 13F whale-following | event-driven, non-price information | 45-day-delayed quarterly disclosures, about 12 tradeable events per year at 5,000, documented post-disclosure drift generously 100 bp per event | 600 | ARITHMETIC, 4x below floor. Also UNTESTABLE here: whale_activity in analysis_bars.db has zero rows, so the named feed cannot be tested under this session's data constraint |
| Turn-of-month equity premium | calendar risk premium | about 40 bp per 4-day window, 12 windows per year, SPY+QQQ at 10,000 | 480 | ARITHMETIC, 5x below floor. Also a STRUCTURAL DUPLICATE of H-A: a calendar-window index long premised on flow-driven compensation |
| FOMC pre-announcement drift | event-driven | documented about 49 bp per event, 8 events per year, 10,000 deployed | 392 | ARITHMETIC, 6x below floor. Also no event calendar exists in analysis_bars.db |
| Crypto-to-equity overnight spillover | cross-asset information | honest documented magnitude about 10 bp net per selected event, about 120 events per year, SPY+QQQ at 10,000 | 1,200 | ARITHMETIC, 2x below floor |
| Equity-to-crypto session spillover | cross-asset information | documented spillover 20 to 40 bp gross per event against the 50 bp crypto round trip | net at or below zero | ARITHMETIC, the fee exceeds the documented move |
| First-half-hour intraday equity momentum | intraday seasonal | generously 7 bp net per day, 252 days, 10,000 deployed | 1,764 | ARITHMETIC, 1.4x below floor at a generous claim, and it is the ten-trades-a-day shape the fee structure punishes |
| Cross-sectional equity momentum or reversal among AAPL, MSFT, NVDA | cross-sectional relative | single-name spread earning 5 to 10 percent per year gross on 10,000 | 500 to 1,000 | ARITHMETIC, 2.5x below floor at best. Also the universe is NVDA-poisoned: queue 002 measured NVDA at 151 percent of the U2 effect, and any long-side pass on 2026-selected names is survivorship |
| Crypto weekend premium | calendar risk premium | 20 to 40 bp gross per weekend against 50 bp fee, 52 per year | net below zero | ARITHMETIC. Also a structural duplicate of H-C (clock-based single-instrument crypto hold) |
| Monthly cross-sectional crypto momentum, 3 majors | cross-sectional relative | literature-faithful majors-only spread 10 to 15 percent per year gross, monthly two-leg turnover costs 12 percent per year at the taker rate | 0 to 300 net | ARITHMETIC, fees eat the documented effect at monthly turnover |
| ETH/BTC band reversion at 2-sigma extremes | cross-sectional relative | not computed | not computed | MECHANISM. The ratio carries a known multi-year secular drift, so band reversion is a countertrend knife-catch, and the rotation mechanism that would sustain it supports trends, not band reversion. No persistence story survives for a fee-paying participant |
| Variance or skew risk premia | risk premium | not computable | n/a | UNTESTABLE, no options data in analysis_bars.db |
| Price-jump event studies (N-sigma day, then continuation or reversion) | event-conditioned own-price | not computed | n/a | STRUCTURE. A price-derived event is still predicting a single instrument from its own recent price history, the exhausted design |

What the floor implies, applied: every equity-side candidate structurally different from the tested four dies on arithmetic, because equity conditional moves at allowed sizing are too small (daily-frequency needs 20 bp net conditional on one position) and the clean structures (cross-sectional single names) are poisoned by the 2026-selected universe. The only shapes that pass arithmetic in this database are multi-day crypto holds with large conditional moves, which then face the 50 bp taker fee. Both survivors below are that shape.

SURVIVING CANDIDATES, two. Both crypto, both multi-day, both stated with capacity at or barely above the floor, so a measured magnitude below claim fails capacity regardless of significance.

### PRE-REGISTRATION (Task 2), committed before any backtest code ran, list closed at commit

H-D. MAJORS-TO-SOL CONVERGENCE (cross-asset lead-lag). STRUCTURAL DIFFERENCE: the signal comes from OTHER instruments (BTC and ETH price history drives a SOL position). All four tested families used only the traded instrument's own history. MECHANISM: information and capital reach the most liquid crypto majors first and diffuse to less liquid alts with delay. Alt flow is retail-dominated and slow, and shorting frictions keep fast money from closing the lag. The claim: after the majors move and SOL lags, SOL converges toward the majors' move over the following days. Documented as the BTC-leads-alts lead-lag and the rotation pattern. SPEC: at each UTC daily close d (close of the day's last 5-minute bar), gap = mean(BTC 5-day return, ETH 5-day return) minus SOL 5-day return, all from daily closes. If flat and |gap| >= 5 percent, enter SOL at the open of the first 5-minute bar of the next UTC day, direction sign(gap): long SOL when the majors are ahead, short SOL when they are behind. Hold exactly 5 UTC days, exit at the open of the first bar of the day after the fifth. One position at a time, no overlap, no stop, no target, the horizon is the exit. Days with no SOL bars produce no signal and no mark and are counted. FEES: crypto taker 25 bp per side, 50 bp round trip. Shorts are not executable on Alpaca crypto, stated: the short side is research, the long-only subset is a registered secondary with no verdict authority. UNITS: 1 unit (5,000) when in market, 0 flat, against the benchmark's constant 1. CAPACITY CLAIM: capture 35 percent of a minimum 5 percent gap, about 175 bp gross and 125 bp net per trade, about 40 non-overlapping trades per year: 40 x 5,000 x 0.0125 = 2,500 USD per year, AT the floor exactly, so any measured shortfall fails capacity. UNIVERSE HONESTY: SOL sat outside the top-10 by capitalization in January 2021 and is a hindsight admission for the fit years, stated now. The holdout (2024 on, SOL top-5 by then) is the cleaner period.

H-E. ETH-BTC ROTATION SPREAD (relative momentum, dollar-neutral). STRUCTURAL DIFFERENCE: the position is RELATIVE, long one leg and short the other, zero net market direction. All four tested families held absolute market direction. The signal family (sign of a trailing return) is shared with H-B and that is stated plainly: what is new is the position space, a composition bet with market beta removed, not the signal family. MECHANISM: capital rotation between the two largest crypto ecosystems is narrative-driven, slow, and shorting-constrained, so relative trends persist for weeks. Cross-sectional momentum is documented in crypto. The claim: the sign of the trailing 56-day ETH-minus-BTC relative return predicts the following week's relative return. SPEC: at each Monday 00:00 UTC close, s = sign(ETH 56-day return minus BTC 56-day return). Hold 5,000 long the leading leg and 5,000 short the lagging leg for the week. Enter or flip at the open of the first bar of the Monday UTC day. No dead band, no intra-week leg rebalancing. FEES: taker 25 bp per side per leg. Initial entry 2 sides = 25 bp of the 10,000 gross. A flip is 4 sides = 50 bp of gross. UNITS: 2 units gross (10,000) deployed continuously against the benchmark's 1. The primary comparison is per unit of GROSS capital, the conservative reading, stated per Task 3's leverage rule. UNIVERSE RULE, hindsight-free: the two largest non-stablecoin cryptos at every formation date since 2021, which selects BTC and ETH at every date in the span. CAPACITY CLAIM: a sign-following spread capturing about 30 percent per year gross on the relative series (roughly Sharpe 0.5 on a 55 percent volatility spread), minus about 3 percent per year in flip fees at an assumed 6 flips: 10,000 x 0.27 = about 2,700 USD per year, barely above the floor.

COMMON MACHINERY. Data: analysis_bars.db only, 5-minute bars. Crypto daily close = close of the UTC day's last bar, entry fills at the open of the next UTC day's first bar, no lookahead anywhere. Daily marks are close-to-close, per-trade returns are fill-to-fill, and the boundary difference (one 5-minute bar) is reported if it exceeds 1 bp. SOL hole verified from bar availability: 2023-07-06 to 2024-08-26, 417 days, spanning the split boundary. BTC and ETH coverage is complete (2,031 days each). Warmup: first signal after the formation window is fully available.

SPLIT AND THE ONCE-TOUCHED HOLDOUT. FIT: days before 2024-01-01. HOLDOUT: days on or after 2024-01-01, evaluated ONCE after fit inspection, zero specification changes in between, result final. CONTAMINATION STATED: the 2024+ crypto period has been read by P26, by P28's H-B and H-C, and by the benchmark session, for OTHER hypothesis families. It has never been read for these two hypotheses. The bar carries the 2-test correction, and any pass with holdout |z| in [2.24, 3.0) is pre-declared to read as passes-the-bar-weakened-by-holdout-reuse, requiring fresh data before any application.

PRIMARY METRIC AND BAR. Per-unit paired daily excess over the BTC/USD buy-and-hold class benchmark, rebuilt from this database with its 50 bp round trip amortized. Strategy daily return per unit of deployed capital, flat days count zero while the benchmark stays invested, the capital-allocation reading. CLUSTERING UNIT: the calendar day. z uses a Newey-West standard error with 7 lags over days (5-day holds and weekly evaluation induce serial dependence). FAMILY: 2 primary tests, Bonferroni family alpha 0.05, per-test 0.025 two-sided, |z| >= 2.24 on the holdout. VERDICT RULE: beats buy and hold requires holdout per-unit excess > 0 at |z| >= 2.24, fit the same sign, AND measured holdout capacity at or above the floor. Interval spanning zero: cannot be distinguished. Interval at or below zero: fails. Measured capacity below floor: fails capacity regardless of z. REGISTERED SECONDARIES, no verdict authority: per-trade net expectancy with SE (clustered by trade, non-overlapping by construction), H-D long-only subset, H-E absolute spread Sharpe. Beside every primary: annualized return, annualized volatility, Sharpe, max drawdown on the cumulative-sum path, time in market, n trades, n days, units deployed, round trip used (50 bp).

CAPACITY RE-CHECK RULE: annual USD = mean daily net strategy return per unit x units x 5,000 x 365, computed on the holdout, floor 2,500.

MECHANISM CHECKS, descriptive, pre-registered, no correction spent, computed for both periods: H-D (a) long against short leg asymmetry, the mechanism predicts the long side carries more, (b) convergence speed, first 2 days against last 3 of each hold, (c) calendar-year means. H-E (a) flip count and realized fee share, (b) calendar-year means, (c) concentration of spread P&L in ratio-trend years.

SAMPLE SIZES, expected: H-D fit about 860 tradeable days, holdout about 695 (the hole removes 2024-01-01 to 2024-08-26), trades 50 to 110 per period at the assumed trigger rate. H-E fit about 148 weeks, holdout about 133. Any registered group under 30 trades reports insufficient_sample and no conclusion, with the holdout still reported once for the record.

WHAT COUNTS AS NEGATIVE, stated in advance: every holdout interval spanning zero, every |z| under 2.24, and every measured capacity under the floor. A negative on both hypotheses is the EXPECTED outcome and will be reported as the headline if it occurs.

RESOURCE RULE: single-process Python over daily aggregates, RSS probed first, run inside a systemd-run MemoryMax scope per the 2026-07-25 convention. Stack checked: DOWN at session start (0 stack processes), 5,986 MB available.

### FINDINGS

**HEADLINE, the expected outcome occurred: NOTHING SURVIVES. Both pre-registered hypotheses fail. H-D (majors-to-SOL convergence) reads -12.5 bp per day against BTC buy and hold in holdout (z=-1.02, interval spanning zero) with a negative point estimate in BOTH periods and the convergence mechanism inverted: SOL diverges after lagging, it does not catch up. H-E (ETH-BTC rotation spread) reads -8.2 bp per day against benchmark in holdout (z=-0.86, spanning zero), its absolute spread return is -1.9 percent per year at Sharpe -0.09, and its gross signal delivered one tenth of the claimed magnitude. Both fail the capacity floor at measured magnitude (-1,695 and -190 USD per year against +2,500 required). Neither failure is a fee story: H-D is gross-negative per trade (-70.6 bp before the 50 bp fee) and H-E grosses about +3 percent per year against a 30 percent claim. The signals are dead, not expensive. This is the sixth consecutive hypothesis family to fail against buy and hold, and the implication section below argues rounds of this kind are exhausted.**

Pre-registration committed at ca504b9 before any backtest code. Fit inspected first, holdout evaluated ONCE after, zero specification changes in between. Runs probed at 83 MB RSS, single process, inside a systemd-run MemoryMax=2G scope.

DATA-HANDLING BUG FOUND AND FIXED DURING FIT INSPECTION, disclosed: the first fit run used pandas pct_change with its deprecated pad default, which forward-fills the SOL hole and fabricated gap signals inside it (15 phantom entries, all landing on missing bars, all excluded, zero P&L effect) and would have fabricated signals from stale padded closes in the first five holdout days after the hole ends. Fixed to fill_method=None so a day with no SOL bars produces no signal, which is what the committed spec says. The fix is toward the registration, not a change of it. Verified: every realized fit number is identical before and after, only the phantom exclusion count moves (15 to 1). The holdout was not touched until after this fix.

PRIMARY TABLE (per-unit paired daily excess over BTC buy and hold, 50 bp crypto round trip inside, NW7 SE, bar 2.24):

| hypothesis / period | days | excess bp/day [95% CI] (z) | strat ann / vol / Sharpe / maxDD | bench ann / vol / Sharpe / maxDD | time in mkt | capacity USD/yr |
|---|---|---|---|---|---|---|
| H-D fit | 913 | -39.72 [-86.32, +6.87] (-1.67) | -120.0% / 115.9% / -1.04 / -395% | +25.0% / 69.1% / 0.36 / -123% | 61.3% | -6,000 |
| H-D holdout | 697 | -12.48 [-36.48, +11.51] (-1.02) | -33.9% / 52.2% / -0.65 / -86% | +11.7% / 44.8% / 0.26 / -68% | 40.9% | -1,695 |
| H-E fit | 1,095 | -9.70 [-31.22, +11.82] (-0.88) | -2.3% / 21.8% / -0.11 / -41% | +33.1% / 65.0% / 0.51 / -123% | 94.6% | -229 |
| H-E holdout | 936 | -8.22 [-27.04, +10.60] (-0.86) | -1.9% / 20.4% / -0.09 / -34% | +28.1% / 47.9% / 0.59 / -68% | 100% | -190 |

UNITS, per Task 3: H-D deploys 1 unit against the benchmark's 1, no leverage artifact possible. H-E deploys 2 gross units continuously against the benchmark's 1 and the table is per GROSS unit, the conservative reading stated in the registration. Drawdowns are cumulative-sum paths, comparable across rows.

H-D SECONDARIES AND MECHANISM, registered: per-trade net -282.4 bp (SE 171.0, z=-1.65, win 43.8 percent, n=112) in fit, -120.6 bp (SE 103.6, z=-1.16, win 43.9 percent, n=57) in holdout. Gross per trade in holdout is -70.6 bp, negative BEFORE fees. The long-only executable subset (registered secondary): +126.2 bp (z=+0.64, n=60) in fit, -198.4 bp (z=-1.63, n=30) in holdout, sign-flipped across periods. MECHANISM CHECKS, all against the claim: (a) asymmetry flips: fit damage concentrated in shorts (-754.0 bp, z=-2.71, SOL kept rising after majors dumped) while holdout damage concentrated in longs (-198.4 bp), the same both-directions sign-flip that convicted H-A's mechanism in queue 002. (b) Convergence speed: the first two days run AWAY from the majors in both periods (fit -181.9 bp, holdout -21.3 bp gross). SOL does not converge, it diverges. (c) Calendar years: all six years negative (2021 -245.7, 2022 -349.5, 2023 -243.5, 2024 -187.5, 2025 -119.3, 2026 -29.0 bp per trade). The lead-lag story is not weakly supported, it is contradicted: the tradeable content of a majors-versus-SOL divergence is continuation, and a continuation bet is the exhausted own-price design this round exists to escape. Excluded on missing bars: 1 fit, 0 holdout, counted per the spec.

H-E SECONDARIES AND MECHANISM, registered: flips ran 8.0 per year in fit and 10.1 in holdout against the 6 assumed, so realized fee drag is about 4 to 5 percent of gross per year against the 3 assumed. Decomposition in holdout: net -0.52 bp per day, fees 1.39 bp per day, gross +0.87 bp per day, about +3.2 percent per year per gross unit against the registered claim of 30. The signal is one tenth of claim, and fees take more than all of it. Calendar years alternate sign with no persistence: fit +13.0, -21.7, +1.9 percent per year (2021 to 2023), holdout -28.5, +27.1, -6.0 (2024 to 2026). The registered concentration check fails its own prediction: 2022, the strongest ratio-trend year in the span, was the WORST spread year in fit (-21.7 percent), the whipsaw shape, not the trend-capture shape.

VERDICTS, in the registered language:
- H-D: FAILS. Holdout interval spans zero with a negative point estimate, fit the same sign, capacity -1,695 USD per year. Does not beat buy and hold, does not clear the floor, and its mechanism is contradicted in both periods.
- H-E: FAILS. Holdout interval spans zero with a negative point estimate, fit the same sign, capacity -190 USD per year. Does not beat buy and hold, does not clear the floor, and its gross magnitude is one tenth of the registered claim.
- Ranked by holdout evidence: nothing survives to rank.

WHAT THIS IMPLIES ABOUT FURTHER ROUNDS OF THIS KIND, stated per Task 5. The capacity gate did its job before any computation: 12 of 14 candidates died on arithmetic or structure, and the gate's own record shows WHY the space is now empty. At Level 1 sizing against live fees, every equity-side structure this database can express dies on arithmetic (conditional moves too small at 5,000 per position), and the only arithmetic-viable shape, multi-day crypto holds with large conditional moves, has now been tested in both remaining structural directions (cross-asset signal, relative position) and both are gross-negative. Six families are now resolved against buy and hold: the default stack, H-A, H-B, H-C, H-D, H-E. FURTHER HYPOTHESIS ROUNDS AGAINST THIS DATABASE ARE NOT WORTH RUNNING: 8 symbols of OHLCV bars at this sizing and fee structure have no untested structure left that passes the arithmetic gate. What would genuinely change the calculus, recorded for the operator: (1) a new information axis the bars do not carry (options surfaces, funding rates, order flow, breadth, or the whale feed once it accumulates enough history in a testable store, none of which exists in analysis_bars.db today), (2) maker execution cutting the crypto round trip to 30 bp, which rescues neither hypothesis here since both are gross-negative, or (3) a different sizing authority, which is not on the table because Level 1 is a hard rule. Until one of those changes, the benchmark session's standing conclusion strengthens: buy and hold after costs remains unbeaten by everything this project has measured, now across six structurally distinct families.

Everything ran against analysis_bars.db read-only. Stack DOWN throughout (0 stack processes at start and end). No engine file, threshold, parameter, or behavior changed. Queue untouched. Nothing applied.

Changes: RETURN.md (pre-registration at ca504b9, findings), PROGRESS.md (dated entry). Research script under gitignored build/research_20260726/capacity_round.py.
Commit message: Capacity-gated hypothesis round, structurally different families, benchmark-relative scoring, holdout touched once, findings only, nothing applied
...
Date: 2026-07-25
Model: Fable 5 (claude-fable-5). The prompt said Sonnet. The session runs on Fable 5 and this line records that.
Prompt summary: harden the research harness after the OOM incident, research harness only, stack already down. Task 1 sweep width from measured RSS, never assumed, refuse oversized batches with numbers. Task 2 research batches under a MemoryMax scope, verified on a deliberately oversized batch. Task 3 the harness streams instead of holding the tape, before and after RSS measured, outputs reproduced exactly. Task 4 the runner sizes down or refuses when the trading stack is running. Task 5 confirm the stale 29.9 MB WAL recovers cleanly. Task 6 tests including mutation tests on the sizing calculation and the refusal path. Task 7 document, commit, push.

Constraints honored: no RiskGate logic, no live-trading gate, no adaptive limit-weakening invariant, no Level 1 risk value, no trading threshold, no strategy parameter, no engine behavior. Live trading stays off. Research harness only.

### FINDINGS

**HEADLINE: the harness now runs the same 3.3M-bar tape in 26,784 kB instead of 1,158,892 kB, a 43x reduction at unchanged wall time, byte-identical output. The sweep runner measures one run before launching any, sizes width from that measurement (3 under the incident's numbers, where 14 was launched), refuses a batch that cannot fit beside a running stack, and runs every batch inside a MemoryMax scope that was verified to kill a runaway without touching anything outside it. The stale WAL checkpointed cleanly with integrity ok.**

TASK 1, WIDTH FROM MEASURED MEMORY. `backtest/sweep.py`. The runner probes its first command under `/usr/bin/time -v` and reads Maximum resident set size. The probe does real work, so nothing is wasted. Width = min(cores - 2, (MemAvailable - stack_reserve) x (1 - margin) / measured_per_run). MARGIN: 0.25, chosen to cover what one probe cannot see: per-cell RSS variance across config variants, allocator overshoot, and page-cache pressure from concurrent sqlite readers. Under the incident numbers (1,159,080 kB per run, about 4.6 GB available) the width computes to 3. The incident launched 14. A batch whose single run cannot fit is REFUSED with the numbers stated, for example: "REFUSED: one run needs 4000 MB, but only 2250 MB is usable (5000 MB available minus 2000 MB stack reserve, minus 25% margin)". There is no hardcoded and no operator-supplied width. Refusal exits distinctly (3) from failure (1), empty batch (2), and missing scope (4).

TASK 2, THE MEMORY BOUND. Batches run inside `systemd-run --user --scope -p MemoryMax=<batch budget> -p MemorySwapMax=0`, where the budget is width x measured_per_run x 1.25. VERIFIED LIVE: a 300 MB allocation inside a 64 MB scope died by the scope's own limit (SIGKILL, exit 137) while the identical allocation outside the scope succeeded untouched, so a runaway batch kills itself and the kernel never chooses a victim across the machine. If no user scope can be created the runner refuses rather than running unbounded, and `--no-scope` exists as a loud explicit override.

TASK 3, THE HARNESS STREAMS. `mal_backtest` now walks the tape in monthly windows read through `Storage::real_bars_in_range` BY IDENTITY, holding one month instead of the whole tape. MEASURED, full 8-symbol deep-history run: before (held tape) 1,158,892 kB peak RSS at 1:16.82, after (streaming) 26,784 kB at 1:18.21. 43x less memory, wall time within 2 percent. At 27 MB per run, even a cores-capped 18-way sweep now needs under 500 MB. OUTPUT IDENTITY: `cmp` reports the full-tape outputs of the held and streaming paths byte-identical (all 3,306,485 bars, 19,740 trades), and a test pins the identity on a fixture with equal timestamps across symbols and a month-boundary crossing. The old path stays reachable as `--hold-tape` so the identity stays testable forever. ONE HONEST NOTE: the old comparator sorted equal timestamps in std::sort's unspecified order, which was measured to be arbitrary per stamp and which no streaming merge can replicate. Both paths now use the defined order (timestamp, symbol). Against the 2026-07-25 emission this shifted cross-symbol tie interleaving: 19,740 trades against 19,648, equity_end 99571 against 99538, a 0.47 percent trade-set drift through book-state coupling only. Per-symbol decision sequences are unchanged and the research conclusions are far outside this drift. Bars lines now emit after the walk (counts accumulate while streaming), which report.py reads position-independently.

TASK 4, RESEARCH NEVER OUTRANKS THE STACK. The runner scans /proc for the four stack components (mal_engine, python_bridge, api_server, ops.watchdog), sums their VmRSS, and reserves that amount AGAIN as free headroom before sizing. DEMONSTRATED BOTH WAYS with the documented `--assume-*` flags, because the stack must stay down this session: stack 1500 MB with 8000 MB available sizes DOWN to width 4 and says "trading stack RUNNING: 1500 MB, sizing around it". Stack 2000 MB with 5000 MB available and a 4000 MB run REFUSES with exit 3 and the message above. Tests pin both paths against the pure sizing function, no simulation flags involved.

TASK 5, THE STALE WAL. With the stack confirmed down, one normal open of market_ai_lab.db: `PRAGMA integrity_check` ok, `PRAGMA quick_check` ok, and the clean close checkpointed the 29.9 MB WAL into the main file and removed it. Nothing else was touched. The week's data through the last heartbeat is intact.

TASK 6, TESTS. `tests/test_research_sweep.py`, 19 tests: the incident numbers compute width 3 and never 14, width scales with available memory (4 cases) and per-run RSS (3 cases), the worker cap and margin bind, an oversized batch refuses with its numbers in the message, the CLI refuses with exit 3, a running stack shrinks width and its reserve is exact, a stack too big refuses a single run, the CLI sizes down and names the running stack, the MemoryMax scope kills a 300 MB runaway inside a 64 MB bound while the control allocation outside succeeds, the probe measures a real 50 MB allocation, streaming and held tape are byte-identical across equal timestamps and a month boundary, and an empty command file exits 2. MUTATION-TESTED both ways with restore: hardcoding width to 14 fails 15 of 19, removing the refusal branch fails 3 of 19, restored passes 19 of 19. FULL SUITE: pytest 986 passed (967 prior + 19 new), ctest 31 of 31. mal_engine was never rebuilt.

Changes: backtest/sweep.py (new), tests/test_research_sweep.py (new), core/backtest_main.cpp (streaming tape source, defined bar order, --hold-tape, bars lines after the walk), RETURN.md, PROGRESS.md, CONTEXT.md. Only the mal_backtest target was built.
Commit message: Size research parallelism from measured memory, bound research under a memory scope, stream the harness, and refuse batches that would crowd a running stack, live trading untouched
...
CRITICAL CONSTRAINT: VIOLATED IN EFFECT. INCIDENT, reported first. No stack process was ever started, stopped, restarted, or signalled by hand, no port bound, no production write, no watched file changed, no running artifact rebuilt. But the 14-way parallel backtest matrix this session launched exhausted host memory, and the kernel OOM killer swept the user session at 19:47:48-58 local (2026-07-26T02:47:48Z), killing the validation stack: engine, bridge, backend, and watchdog all died between the engine's last heartbeat at 02:47:44Z (iteration 2457, equity 99999.7) and the sweep's end. The full incident section below carries the evidence and the state left behind. The stack is DOWN and was LEFT DOWN: the same prompt forbids starting processes, and restarting the validation week is the operator's decision by established precedent.
...
**HEADLINE: the question the first run could not answer is now answered. At 19,648 trades, 54.7 times the first run's sample, the default strategy's pooled net expectancy is NEGATIVE and statistically resolved: -1.97 bp per trade, 95 percent interval [-2.68, -1.27], excluding zero for the first time. The magnitude equals the measured 2 bp round-trip fee. Gross expectancy is indistinguishable from zero. The strategy is not an edge being filtered too hard. It is a zero-gross-edge trade generator paying costs. Every gate, every threshold cell, every frequency lever measured confirms the same shape. No gate rejects winners. There are no winners being rejected.**

Machinery checks, before any number. The extended harness binary reproduced the pinned slice decision-identically (1,079 records equal). The Python counterfactual walk reproduced the harness's own 19,648 C++ trades at 99.98 percent exit-reason agreement with 0.013 bp mean absolute return deviation, far inside the pre-registered 95 percent / 0.5 bp gate, so the counterfactual numbers below are load-bearing. All 62 planned runs completed except trigger_off, which config validation itself refuses (RSI-2 entry must sit in (0,100) below the exit), a finding in its own right: the trigger cannot be disabled by any operator config, so it was measured by the counterfactual walk as pre-registered for trend_filter.

TASK 1, WHAT EACH GATE COSTS AND EARNS. Fire rates across 3,178,866 evaluated no-signal bars, then the marginal rejected set (all other conditions pass, only this gate fails) walked through the strategy's own exit semantics. Accepted set through the same walk: -1.98 bp, se 0.36. Bonferroni bar |z| >= 2.89.

| gate | first-refusal count | marginal n | rejected-set bp [95% CI] | vs accepted z | verdict |
|---|---|---|---|---|---|
| no_ema_cross | 1,364,589 | 1,969,634 (walked 39,392 sampled) | -0.21 [-1.34, +0.92] at 24-bar horizon | n/a (trigger, no direction) | trigger, not a filter. Non-cross bars drift at zero. The cross does not select either. |
| rsi2_trigger | 915,611 | 384,368 (walked 38,436 sampled) | -1.81 [-2.09, -1.54] | +0.42 | DOES NOTHING. Rejected dips perform exactly like accepted dips. |
| trend_filter | 861,194 | 26,746 | -1.71 [-2.23, -1.19] | +0.60 | DOES NOTHING measurably. Its one real effect is structural: it removes 48.8 percent of the tape (below-MA200 bars) from play entirely. |
| atr_band | 18,184 | 15,123 | -1.24 [-1.81, -0.67] | +1.59 | DOES NOTHING at the bar. The lean is the wrong direction: its rejects are the least-bad set measured. Removal arm agrees (+0.49 bp pooled, z=1.09). |
| volume | 8,264 | 14,581 | -1.68 [-2.28, -1.09] | +0.62 | DOES NOTHING in SIP terms. Removal arm delta z=-0.04, the flattest result in the session. SIP-only: this does NOT transfer to the live IEX path, which carries roughly 5 percent of consolidated volume and mostly no volume at all. |
| dual_ma | 5,277 | 8,387 (4,330 long-side walked) | -3.74 [-5.75, -1.73] | -1.62 | the ONLY gate whose rejects lean worse than accepted. Removal arm agrees (added 3,566 trades at -4.87 bp). Not significant at 2.89, so formally indistinguishable, but it is the only gate with a protective direction. |

Two facts dominate the table. First, every rejected set has a negative mean. No gate is discarding profitable setups, so no loosening can recover hidden expectancy. Second, no gate separates from the accepted set at the corrected bar. The gates spend 2,238 live rejections a week sorting candidates that all earn the same minus-fee return.

TASK 2, THRESHOLD SENSITIVITY. 54 cells, all completed, fit period before 2024-01-01, holdout after. The full surface: every one of the 54 cells is negative in fit AND holdout. Fit range -3.31 to -1.46 bp, holdout range -2.32 to -1.44 bp, against per-cell standard errors near 0.4 bp. Zero positive cells out of 108 cell-periods. The pre-registered selection (best fit cell, E15/T200/S1/X80 at -1.46 bp) FAILED holdout confirmation against the default cell: delta -0.07 bp, z=-0.14. Pre-declared negative confirmed: the surface is flat within noise, threshold tuning is not where an edge lives, and any cell reported alone would have been noise-mining. Trade count moves 9,334 to 16,274 across the grid, so thresholds do buy frequency, at an unchanged negative per-trade return.

TASK 3, MOMENTUM AT 5 MINUTES. n=6,383 momentum trades: -2.36 bp [-4.24, -0.49], win rate 41.0 percent, median hold 10 bars. Exits: 50 percent stop, 27 percent target, 23 percent time-stop. Stopped within one bar: 227 trades, 3.6 percent of momentum entries. Within three bars: 12.2 percent. The unconditional noise floor (any bar dipping 2x ATR within 3 bars, no signal conditioning): 10.2 percent crypto, 11.4 percent equity. Conditional 12.2 against unconditional 10 to 11: momentum entries are barely more stop-prone than random bars. Median adverse excursion in the first 3 bars is 44 percent of the stop distance. Per the pre-registered rule: the 2x ATR stop is NOT inside normal bar noise, entry timing is NOT adverse-selecting, and the live week's three one-bar stop-outs read as unlucky samples from a 3.6 percent base rate on a choppy tape, not a placement or timing defect. The momentum factor's problem is not its stop. It is that its trades earn -2.4 bp.

TASK 4, THE FOUR FREQUENCY LEVERS, ranked by expectancy-adjusted contribution. Every lever multiplies a roughly -2 bp per-trade expectancy, so every lever as measured makes the account lose faster. Ranked by measured per-trade effect:

1. TIMEFRAME (15-minute): the only lever whose point estimate improves per-trade return, -0.88 bp [-2.87, +1.12] against -1.97, delta +1.10 bp, z=1.01, NOT significant, and it REDUCES trade count to 7,015 (fit +0.73 / holdout -2.93 flips sign across periods, so even the lean is unstable). 1-minute is unanswerable this session: the analysis DB holds 5-minute bars and pulling years of 1-minute history would contend with the live feed's Alpaca rate limit mid-week.
2. SHORTS: structurally completes the half of the tape the stack cannot touch (48.8 percent of bars below MA200, 5,747 live-style short_not_allowed refusals, 9,209 blocked short candidates). Measured: exact crypto momentum shorts (config flag) 3,045 added shorts at -0.56 bp [-4.04, +2.93], z=+0.28 vs longs. Full mirrored stack on the inverted tape: -1.93 bp [-2.69, -1.16] pooled, crypto -1.14, equity -3.32, z=+0.08 vs the long side. The short side loses the same way the long side does. RiskGate and sizing would need a short-position convention (negative qty or a side field through OrderProposal, exposure and stop-distance math already sign-aware, sizing unchanged), none of it implemented or needed on this evidence.
3. THRESHOLD LOOSENING: buys 9,334 to 16,274 trades across the grid at unchanged negative expectancy (Task 2). Frequency without expectancy.
4. UNIVERSE: per-symbol rates are 2.1 to 2.3 trades/day for crypto, 0.5 to 0.7 for equities, additive by construction, so a 16-symbol core would roughly double frequency. But per-symbol expectancy is negative for 7 of 8 symbols (only SOL/USD positive at +2.75 [-0.12, +5.61], spanning zero, and SOL carries the survivorship caveat most strongly). Adding symbols at these thresholds adds losing trades fastest. The worst lever.

TASK 5, STRATEGY DIAGNOSIS. The two factors do not fight. They overlap. Momentum longs require price above MA50 AND MA200. RSI-2 reversion requires price above MA200. Both are long-only in practice. The stack is one bet expressed twice: "price above its 200-bar average keeps rising", once as cross-following, once as dip-buying. Consequences, measured: 48.8 percent of evaluated bars sit below trend and can produce NOTHING, which is why a downtrending live week produced 3 trades, and why the frequency complaint and the strategy design are the same fact. The regime detector's weighting is unsupported by outcomes: in trending regime it weights momentum 0.70 but momentum earns -2.00 there against reversion's -1.78, in neutral it splits 50/50 but momentum earns -3.08 against -1.79, and range_bound momentum fired once in 7 years. Reversion beats momentum in every regime with enough sample, and both lose. Winners hold a median 3 bars, losers 4: exits truncate winners marginally faster than losers. The atr_band leans toward rejecting the least-bad candidates (violent tape is where dip-buying pays best, and the band excludes it), directionally self-defeating though not significant. VERDICT, stated plainly: on 61x the production tape, with an optimistic selection bias, this strategy shows NO GROSS EDGE (accepted and every rejected set all sit at zero gross, minus fees net) and a statistically resolved NEGATIVE net expectancy of -1.97 bp [-2.68, -1.27] per trade. That is not "cannot be distinguished from no edge" any more. It is distinguishable, and the answer is no edge at 5-minute granularity for this stack on this universe. Raising trade frequency in any form multiplies a negative number.

TASK 6, THE RESEARCH LOOP ITSELF. Measured this session: one full-span run is 120 s single-threaded and CPU-bound (DB read is about 4 s of it), so the cost is the per-bar O(window) indicator recomputation in strategy::evaluate, which recomputes 300-bar EMA/RSI/ADX/ATR series every bar. The 62-run matrix completed in about 12 minutes at 14-way parallelism through a one-line xargs runner, so sweeps already parallelize embarrassingly. analysis_bars.db is correctly indexed for the harness's exact query (idx_bars_lookup on symbol, timeframe, timestamp), nothing to add. The emit-rejections stream is 1.2 GB per run and streams through Python in about 4 minutes. One hypothesis end to end (config copy, run, report) is about 5 minutes. What bit this session: a SIGABRT in one cell (the invalid trigger_off config) made xargs stop scheduling the rest of the matrix, and a session restart lost all in-flight runs because outputs are truncated on open. Recommendations, by expected saving: (1) incremental indicator state in the harness bar walk, expected 10 to 50x on the 120 s run, the single biggest lever, but it must reproduce the pinned baselines bit-for-bit before use since it touches evaluation order; (2) keep the fault-tolerant runner pattern (wrap each cell so one abort cannot starve the sweep), zero cost, done this session; (3) a marginal-set filter flag on --emit-rejections, expected 10x output reduction; (4) content-addressed run caching keyed on (binary hash, config hash, db, span) so an interrupted sweep resumes instead of restarting, worth roughly one full sweep per interruption.

EXPLORATORY (not pre-registered, labelled per Task 0): the equity/crypto asymmetry (equity -3.49 vs crypto -1.08) and SOL/USD's positive lean were observed, not tested, and carry the strongest survivorship caveats. The live-week fire-rate ordering (trend_filter, no_ema_cross, rsi2_trigger dominating rejections) reproduces on deep history (no_ema_cross 1.36M, rsi2_trigger 916k, trend_filter 861k of 3.18M evaluated bars), so the live week's gate behavior is the strategy's normal behavior, not an anomaly.

RANKED RECOMMENDATIONS (apply nothing, each with its confirming observable):
1. STOP TUNING THIS STACK. The resolved finding is no gross edge at 5-minute granularity. Threshold, gate, short, and universe changes all multiply -2 bp. CONFIRMING OBSERVABLE: any future pre-registered run on new out-of-sample tape showing a pooled interval excluding zero on the positive side would overturn this. Nothing else would.
2. If frequency is still wanted for pipeline-exercise reasons (council, tuner, fill plumbing), take it from the cheapest lever, threshold loosening, with position size cut proportionally, and account it as an operating cost of about 2 bp per trade, stated as such, never as strategy improvement. CONFIRMING OBSERVABLE: live per-trade net return stays within the harness interval while fill count rises.
3. The only structurally new information channels left for this universe are (a) a timeframe redesign around 15-minute-plus bars where the point estimate at least improves, and (b) signals not derived from the same price series (the council, whale flow, cross-asset), which this harness deliberately cannot test. Both are redesigns, not tunings. CONFIRMING OBSERVABLE: a pre-registered 15m-native or signal-augmented run with positive holdout interval.
4. Adopt the Task 6 items in order (incremental indicators, fault-tolerant runner, filtered emission, run caching). CONFIRMING OBSERVABLE: full-span run under 10 s, sweep restart resuming from cache.

INCIDENT: THE RESEARCH LOAD KILLED THE VALIDATION STACK. Discovered by this session's own end-of-run verification, diagnosed read-only, nothing restarted.

Timeline, local time (UTC in parens):
- 19:47:0x. The 62-run matrix launched at 14-way parallelism. Each full-tape mal_backtest peaks at 1.13 GB RSS (measured afterward: Maximum resident set size 1,159,080 kB), so the matrix demanded roughly 15.8 GB on a 14 GB host that had about 4 GB available. This per-run footprint was never measured before launch. That omission is the root cause.
- 19:44:06 (02:44:06Z). Last journal event, id 5561, a routine discovery_skip. Nothing after it.
- 19:47:44 (02:47:44Z). Last engine heartbeat: pid 2908051, iteration 2457, equity 99999.7.
- 19:47:48-58 (02:47:48-58Z). systemd-oomd invoked the kernel OOM killer, repeated sweeps: journal records session.slice and app.slice "The kernel OOM killer killed some processes in this unit", named victims include claude-desktop and steamwebhelper. The engine, bridge, backend, and watchdog all died in this window. No stack process recorded anything: no engine_stop, no continuous_stop, no watchdog condition, no process_stop, and watchdog.log has no teardown lines. That silence is the SIGKILL signature. A 29.9 MB market_ai_lab.db-wal remains, an unclean close (SQLite recovers the WAL on next open, no corruption expected).
- ~20:0x. The remaining 48 runs relaunched after the session restart and completed WITHOUT incident, on a host the first sweep had already stripped of several GB. Luck, not design.

State left behind, verified read-only: all position rows carry qty 0.0 (the engine died flat), last-heartbeat equity 99999.7, the week's 6 recorded trades and full journal through 02:44:06Z intact. Ports 8765 and 8000 free. The stack was found down and LEFT DOWN: starting processes is forbidden by this prompt, and restarting the validation week is the operator's decision, the precedent set 2026-07-25. The operator's normal start script is the restart path.

Prevention, added to the Task 6 recommendations: size sweep parallelism from measured memory, P = min(cores - 2, available_mem / measured_per_run_RSS), which for this matrix would have been 3, not 14. Better, run research batches inside a bounded scope (systemd-run --user --scope -p MemoryMax=...) so the OOM victim is always the research, never the stack. The harness could also cut its footprint 5 to 10x by streaming bars per symbol instead of holding the merged 3.3M-row tape.

Everything else the constraint names was honored: no process was deliberately started, stopped, or signalled. No port bound. market_ai_lab.db opened SELECT-only, zero writes. controls.json, default_config.yaml, and every file the running engine reads unchanged. mal_engine was not rebuilt (binary mtime Jul 24 21:58 before and after). All computation ran against analysis_bars.db and derived gitignored copies. None of that unkills the stack. The constraint's purpose was the stack's survival, and the session failed it.

Changes: RETURN.md (pre-registration committed before any run, then these findings), PROGRESS.md (dated session entry), core/backtest_main.cpp (additive research emission on reject and trade lines, validated decision-identical on a pinned slice, mal_backtest target rebuilt alone). All run outputs, configs, derived DBs, and analysis scripts live under gitignored build/research_20260725/.
Commit message: Pre-registered deep-history research on gate contribution, threshold sensitivity, frequency levers, and overall strategy diagnosis, findings only, live run untouched
...
Date: 2026-07-24
Model: Fable 5 (claude-fable-5)
Prompt summary: Probe Alpaca coverage per core symbol before pulling, prove split adjustment, report feed and volume semantics, load deep 5-minute history into a separate analysis database, tag provenance, report gaps and biases, keep production bars and the pre-flight baseline frozen, change nothing about the harness unless forced and recalibrate if forced, ask no strategy question.

**HEADLINE: 3,306,485 provenance-tagged 5-minute bars now sit in `analysis_bars.db`, 61 times the production tape, with split adjustment proven at both known corporate actions. The production bars table is byte-frozen and every baseline value matches. The freeze check itself caught and fixed a real defect: the control-precedence setter tests were journaling control_change events into the PRODUCTION database on every pytest run. No strategy question was asked.**

TASK 1, AVAILABILITY PROBED BEFORE PULLING. Equity 5-minute SIP history begins 2016-01-01 for SPY, QQQ, AAPL, MSFT and 2016-01-04 for NVDA, established by `/v2/stocks/bars?timeframe=5Min&start=2015-01-01&limit=1&feed=sip` returning its first served bar. Crypto begins 2021-01-01T06:00Z for BTC/USD and ETH/USD and 2021-01-01T06:05Z for SOL/USD via `/v1beta3/crypto/us/bars`, so SOL reaches exactly as far back as BTC on Alpaca. Documented coverage was not assumed. Spans stated before pulling: equities from 2019-01-01 (covers the COVID crash, both splits, the 2021 bull, the 2022 bear, the 2023-2024 bull), crypto from its true start, end frozen at 2026-07-24T00:00:00Z. Projected 3,648,000 rows and ~474 MB. Actual: 3,306,485 rows, 652 MB on disk (indexes and page overhead, and per-symbol bars per session vary below the SPY-measured 192).

TASK 2, SPLIT ADJUSTMENT PROVEN, DIVIDENDS DECIDED. Alpaca's parameter is `adjustment` (raw, split, dividend, all). The existing 30-day backfill sends none, which means raw. The parameter was proven live before the pull: NVDA daily raw shows 1208.88 then 121.79 across 2024-06-10 (the fictional 90 percent crash), split-adjusted shows 120.89 then 121.79. The pull used `adjustment=split`. Dividends are NOT adjusted, by choice: dividend adjustment shifts every historical price away from the level that actually traded, the live path trades raw prices, and typical dividend steps sit far below the strategy noise floor, so split-only matches live semantics while removing the catastrophic artifacts. PROOF IN THE PULLED SERIES: NVDA 10-for-1 on 2024-06-10, last close before 120.78, first open after 120.80, gap +0.02 percent where unadjusted would show -90.0. AAPL 4-for-1 on 2020-08-31, close 125.28 to open 128.00, gap +2.17 percent (a real weekend move) where unadjusted would show -75.0. Both pass.

TASK 3, FEED AND VOLUME SEMANTICS, A BOUNDING FINDING. Historical equity bars: SIP, explicitly requested and confirmed entitled. The account's historical default is also SIP: the production backfill's stored SPY hour sums 4,020,740 volume against the SIP probe's 4,300,554 and the IEX probe's 235,974. The live path is IEX: the latest-bar endpoint's default bar is identical to `feed=iex`, and `feed=sip` on it returns 403 subscription does not permit querying recent SIP data. IEX carried 5.5 percent of consolidated volume in the probe hour. VERDICT, stated plainly: the backfill and live equity volume series are NOT comparable. A volume filter measured on deep SIP history describes SIP volume only. The pre-registered volume question is BOUNDED by this: deep history can answer it in SIP terms, and the live path measures a different quantity until the live feed changes. Second finding: crypto volume presence collapses in recent years because Alpaca's own crypto venue thinned. Bars with nonzero volume: BTC 100 percent in 2021-2022 but 31.8 percent in 2024, ETH 20.2 percent in 2024, SOL 6.6 percent in 2024. Equity SIP volume presence is 100 percent in every year.

TASK 4, SEPARATE DATABASE, PRODUCTION FROZEN, AND WHAT THE FREEZE CHECK CAUGHT. The load writes only `analysis_bars.db` (gitignored via *.db) and the loader refuses the production path outright. The harness selects its source with `--db` and now names it in output: `mal_backtest` emits a meta line with the database path plus the analysis_meta table when present (loader, endpoints, feed, adjustment, pull end, bias note), and `backtest/report.py` surfaces it as `source` in every report. Production content: bars 95,549, trades 253, signals 12,925, entry_decision 12, positions all flat, every value equal to the pre-flight baseline. The FILE hash moved, and diagnosis found the cause: 15 control_change events (ids 5108-5122, 20:27:09Z, source gui) appended during this session's pytest run by tests/test_control_precedence.py, whose setter tests isolate MAL_CONTROL_DIR but not MAL_DB_PATH, so controls._audit journaled through store.append_event into production. The identical burst at 08:03:54Z (ids 5093-5107) matches the previous session's pytest run. Same defect class as the 2026-07-24 watchdog leak. Fixed with the same autouse isolation fixture, and proven: pytest rerun passed 917 with the production event high-water mark unmoved at 5122. The appended rows are audit journal entries only, no bar, trade, position, or control state changed, so the captured baseline VALUES stand and the validation week is unaffected. Post-fix reference hash: 99d66b8162d5bd66dbc2c87e18a0d5c5bfb17c808e42e2297f3421ff79cdddc0.

TASK 5, PROVENANCE, GAPS, QUALITY, SAMPLE. Every loaded bar carries source backfill under the existing provenance system, volume_source NULL. Nothing was filled. SOL/USD has a 417-day hole from 2023-07-06 to 2024-08-26, the Alpaca delisting era, left empty and flagged: any SOL result must segment around it. Equities' longest no-bar gap is 4 days (holiday weekends). Equity session counts above 252 per year come from UTC date bleed: post-8pm-ET bars land on the next UTC date. Usable bars after exclusions: BTC 578,031, ETH 579,744, SOL 457,250, SPY 353,540, QQQ 354,161, AAPL 343,422, MSFT 312,547, NVDA 327,790, total 3,306,485 against 53,872 production usable, 61.4 times. Naive linear projection from the first run's 359 trades: roughly 22,000 trades, against the roughly 9x sample the noise floor required. Per-arm standard error projects from 2.3 basis points toward roughly 0.3.

TASK 6, THE BIASES THE DATA CANNOT REMOVE. Selection and survivorship: these 8 symbols were chosen in 2026 for being liquid and prominent, so testing them through 2019-2026 measures a strategy on names that turned out to be winners. Expected direction: optimistic, and largest where the winning was most extreme (NVDA rose roughly 30x over the span). The exact statement ships in analysis_meta and appears in every harness report via the meta line. Regimes covered: the 2019 grind-up, the 2020 crash and rebound, the 2021 bull, the 2022 equity and crypto bear, the 2023-2024 recovery and NVDA parabola, 2025-2026 as recorded. Not covered: any 2008-scale systemic crisis, pre-2019 volatility regimes, crypto's pre-2021 cycles, and SOL's missing 2023-2024. A later session must segment by regime rather than pool: a single number over this span describes no market that ever existed.

TASK 7, INSTRUMENT CHANGES REPORTED AND THE CALIBRATION RE-RUN. The harness changed in three places, all reported: the meta source line in core/backtest_main.cpp (read-only sqlite open of analysis_meta), the source passthrough in backtest/report.py, and one missing-initializer warning fix in the Book constructor (behavior identical, explicit zero). Because the instrument changed, the calibration was re-run: 6 of 6 recorded live decisions reproduce, the same rows P23 calibrated. pytest 917 passed (matches baseline), ctest 30 of 30, zero build warnings, offline synthetic baselines identical (swing Trades=108 Blocked=204 Events=1222, active_quant Trades=6 Blocked=2 Events=35). WHAT THE NEW SAMPLE ENABLES: sub-basis-point pooled resolution, per-regime segmentation with real per-fold samples, and the pre-registered questions at a sample that can actually resolve them, in SIP volume terms. WHAT IT STILL CANNOT RESOLVE: council-tier behavior, the volume filter as the live IEX path experiences it, expectancy free of the 2026 selection bias, and SOL continuity across its hole. No pre-registered question was asked in this session.

Changes: scripts/deep_history_20260724.py (probe, load, validate), core/backtest_main.cpp (meta line, warning fix), backtest/report.py (source passthrough), tests/test_control_precedence.py (production DB isolation fixture), analysis_bars.db created locally (gitignored), PROGRESS.md, CONTEXT.md.
Commit message: Load and validate deep historical bars into a separate analysis database, production frozen, no question asked yet, live trading untouched
Commit message:
