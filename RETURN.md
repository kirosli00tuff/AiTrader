...
THE RUNNING STACK WAS NOT TOUCHED. No process started, stopped, or signalled. No port bound. market_ai_lab.db opened SELECT-only, zero writes. controls.json, default_config.yaml, and every file the running engine reads unchanged. mal_engine was not rebuilt (binary mtime Jul 24 21:58 before and after). All computation ran against analysis_bars.db and derived gitignored copies.
